...
class UMItoolsAppFastq(BaseSpaceNativeApp):
    ...
    def do_work(self, workspace_directory, appsettings, output_builder):
        ...
        bowtie_index_command = " ".join(["/bowtie-1.2.2-linux-x86_64/bowtie-build","-f",ref_genome_fa,bowtie_index_directory_path])
        ...
        for sample in appsettings.input["sample-id"].content:
            ...
            for fastq_set in sample.fastq_sets:
                ...
                umi_tools_command = ["/usr/bin/umi_tools",
                                     "extract",
                                     "--log="+log_file_path,
            ...
            if fastq_set.is_paired:
                ...
                bowtie_command = " ".join(["/bowtie-1.2.2-linux-x86_64/bowtie","--threads 4 -v 2 -m 10 -a",str(bowtie_index_directory_path),"-1",read1_files,"-2",read2_files,"--sam >",mapped_sam_path])
            else:
                read1_files = ",".join(read1_fastq_files)
                print "read1_files is "+read1_files
                bowtie_command = " ".join(["/bowtie-1.2.2-linux-x86_64/bowtie","--threads 4 -v 2 -m 10 -a",str(bowtie_index_directory_path),read1_files,"--sam >",mapped_sam_path])
            ...
            dedup_command = " ".join(["/usr/bin/umi_tools","dedup","-I",sorted_bam_path,"--output-stats=deduplicated -S",dedup_bam_path])
...
if __name__ == "__main__":
    ...
    app = UMItoolsAppFastq(input_dir, output_dir, scratch_dir, logs_dir)
